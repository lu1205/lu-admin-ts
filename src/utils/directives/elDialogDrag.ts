...
const draggable: ObjectDirective = {
  mounted(ele: HTMLElement, binding: DirectiveBinding<any>) {
    const el = <HTMLElement>document.querySelector('.el-dialog')
    const elHeader = <HTMLElement>document.querySelector('.el-dialog__header')
    // 设置目标元素基础属性
    ...
    // 存在父级容器
    if (containerId) {
      const containerEle = <HTMLElement>document.getElementById(containerId)
    ...
    // 鼠标在目标元素上按下
    elHeader.addEventListener('mousedown', (e: MouseEvent) => {
      console.log(1)
...
function getScrollWidth() {
  ...
  return exist ? noScroll - scroll : 0
}
